...
@Component({
  selector: 'mix-kontakte',
  templateUrl: './kontakte.component.html',
  styleUrls: ['../kaz/kaz.component.scss'],
  ...
  animations: [
    ...
    trigger("FoldOut", [
      transition("void => app", [style({ zIndex: "0" }),
      ...
      transition("app => void", [style({ opacity: "0" }),
...
export class KontakteComponent implements OnInit, OnDestroy {
  ...
  screenWidth: number
  column_no = new BehaviorSubject<number>(1)
  maximum_col: number
  ...
  kategorie = new BehaviorSubject<string>('')
  ...
  constructor(
    ...
    this._subs.sink = this.api.getRubrik('kontakte').subscribe( data => this.data$.next(data) )
    ...
    var segs = 50
    if (this.globals.first_kaz_search == true) segs = 2000
    setTimeout(() => {
      this.getMail()
      this.getPin()
  ...
  goKategorie(rubrik): void {
    ...
    if (this.kategorie.getValue() === rubrik) {
      setTimeout(() => {
  ...
  getImage(image: string) {
    ...
    if ((this.column_no.getValue() < 2) && (this.loaded_images == false)) {
  ...
  getBanner(banner_url) {
    return this._sanitizer.bypassSecurityTrustStyle(`url(http://local.mix.amvara.de:8081/banner/${banner_url})`);
  }
